...
`dll` в проекте VS2019 надо подключать самим, то есть положить их в папку `debug` рядом с `*.exe`.
SDL должен находиться в `С:/SDL2`, в котором должны быть вложены папки `SDL2_ttf-2.0.15`, `SDL2_image-2.0.4` не считая папок самого `SDL2`. В любом случае рекомендуется проверить свойства проекта.

Для людей с `code::blocks`, надо создавать новый проект с SDL2 и закидывать туда все эти файлы, не теряя папок.
